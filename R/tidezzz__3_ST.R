...
tables.st.ren <- rinejm(tables.st)

dfst <- bind_rows(tables.st.ren) %>% as_data_frame()
...
dfst.sep.date <- add_column(dfst.sep, dan = date, .before = 1) %>% 
  select(-Day)

dfst.sep.date.vis <- dfst.sep.date %>%
  mutate_at(vars(ends_with("_h")), convert_visina)

dfst.sep.date.vis.dttm <- dfst.sep.date.vis %>% 
...
tidyyy.mijene <- function(df = dfst.sep.date.vis.dttm, mijena, atribut){
  df %>% 
...
saveRDS(mijene, here("podaci", "mijene_tidy_ST.Rds"))

# solunarni

lasci <- dfst.sep.date.vis %>%
  unite("sunrise_tt", dan, Sunrise, remove = FALSE) %>% 
  unite("sunset_tt", dan, Sunset, remove = FALSE) %>% 
  mutate_at(vars(ends_with("_tt")), convert_time_custom) %>% 
  select(dan, sunrise_tt, sunset_tt)

saveRDS(lasci, here("podaci", "__lasci_tidy_ST.Rds"))
